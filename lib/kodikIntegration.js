  ...
  async getVideoFromEpisode(episodeData, options = {}) {
    try {
      ...
      const promises = kodikPlayers.map(async (player) => {
        ...
        // Базовая информация о плеере (всегда возвращаем)
        const baseInfo = {
        ...
        // Если прямые ссылки не получены - возвращаем хотя бы базовую информацию
        console.log(`⚠️ Возвращаем fallback для ${baseInfo.team}`);
        return {
          ...baseInfo,
          directLinksAvailable: false,
          message: 'Direct links unavailable, use kodikLink'
        };
      });
