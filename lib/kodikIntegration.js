...
class KodikIntegration {
  ...
  async getDirectVideoLinks(kodikLink, options = {}) {
    try {
      ...
      // Добавляем таймаут для запроса (8 секунд для Vercel)
      const timeoutPromise = new Promise((_, reject) => 
        setTimeout(() => reject(new Error('Timeout: Kodik request took too long')), 10000)
      );

      // Пробуем получить прямые ссылки с таймаутом
      ...
      try {
        const timeoutPromise = new Promise((_, reject) => 
          setTimeout(() => reject(new Error('Timeout: Fallback request took too long')), 8000)
        );
      ...
      const promises = kodikPlayers.map(async (player) => {
        ...
        try {
          ...
          if (videoInfo && Object.keys(videoInfo).length > 0) {
            ...
            return null; // Не возвращаем плеер без прямых ссылок
          }
        } catch (playerError) {
          console.warn(`⚠️ Ошибка получения прямых ссылок для ${baseInfo.team}:`, playerError.message);
          return null; // Не возвращаем плеер при ошибке
        }
      });

      // Ждём завершения всех запросов параллельно и фильтруем только успешные
      const results = (await Promise.all(promises))
        .filter(result => result !== null && result.directLinksAvailable === true)
